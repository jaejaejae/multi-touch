...
rangeTangent.prototype.checkCordinates = function(i, x, y) {
	...
	else if(Math.sin(this.angles[i]) * Math.sin(y/length) < 0 || Math.cos(this.angles[i]) * Math.cos(x/length) < 0)
		return false;
	else
		return true;
	// body...
...
function touchMoveHandler(range, tag, tagsObj, event){
    ...
    if (event.targetTouches.length == 1) {
      object.style.webkitTransition = 'left ease 0s';
        ...
        tag.rank = tag.updateRank(length);
...
function collisionCase(rect1, rect2){
	...
	var rect1X = Number((rect1.div.style.left).substr(0, (rect1.div.style.left).length-2));
	var rect1Y =  Number((rect1.div.style.top).substr(0, (rect1.div.style.top).length-2));
	var collided = false;

	if(rect2 != rect1){
		var rect2X = Number((rect2.div.style.left).substr(0, (rect2.div.style.left).length-2));
		var rect2Y = Number((rect2.div.style.top).substr(0, (rect2.div.style.top).length-2));
